"""
PINN — équation des ondes 1D.

But : à partir de M_BACK instants PASSÉS (chacun décrit par u_dot et u_xx sur un
voisinage ±SS), prédire N_FWD instants FUTURs (les delta_u aux horizons n+ndt, ...).
On entraîne le réseau, puis on déroule (rollout) sa propre prédiction pour rejouer
toute la propagation de l'onde et mesurer l'erreur.
"""
...
Nx   = 100
SS   = 7
Ntot = Nx + 2*SS                  # grille étendue (zéro padding aux bords)

nodes = np.arange(SS, Ntot-SS)    # indices des nœuds physiques (intérieur)
...
# ============================================================
all_dfs = []
FIELDS = {}          # champ complet u(t,x) de chaque simulation
...
C2  = E / rho     # vitesse d'onde au carré
TAU = ndt * dt    # pas de temps effectif du réseau

IDX_UDOT = INPUTS.index("u_dot(t,j)") 
IDX_UXX  = INPUTS.index("u_xx(t,j)")

RES_SCALE = C2 * float(sd_in[IDX_UXX])

def physics_loss(X_norm, pred_norm):
    udot   = X_norm[:, IDX_UDOT] * float(sd_in[IDX_UDOT]) + float(mu_in[IDX_UDOT])
    uxx    = X_norm[:, IDX_UXX]  * float(sd_in[IDX_UXX])  + float(mu_in[IDX_UXX])
    delta1 = pred_norm[:, 0]     * float(sd_out[0])       + float(mu_out[0])
    u_tt   = (delta1 - TAU * udot) / TAU**2  
    residu = u_tt - C2 * uxx                 
    return ((residu / RES_SCALE) ** 2).mean()
...
# ============================================================
# ENTRAÎNEMENT  (loss données + bruit + physique PINN)
# ============================================================
with torch.no_grad():
    plancher_phys = physics_loss(torch.tensor(X_train[:5000]), torch.tensor(y_train[:5000])).item()
print(f"Loss physique des données vraies (plancher) : {plancher_phys:.4e}")
...
for epoch in range(1, N_EPOCHS + 1):
    ...
    for X_batch, y_batch in train_loader:
        ...
        # --- Loss physique (PINN) : la prédiction doit respecter u_tt = c²·u_xx.
        #     On utilise les features PROPRES (X_batch, pas X_in bruité) : même
        #     philosophie que la cible propre — le réseau doit produire une sortie
        #     physiquement cohérente même depuis des entrées abîmées. ---
        if LAMBDA_PHYS > 0:
            phys_loss = physics_loss(X_batch, prediction)
        else:
        ...
        perte_phys_total += phys_loss.item()

    perte_train      /= len(train_loader)
    perte_phys_total /= len(train_loader)
    ...
    historique_phys .append(perte_phys_total)

    if epoch % 2 == 0:
        print(f"Epoch {epoch:4d}/{N_EPOCHS}  —  "
              f"data: {perte_train:.4f}  |  phys: {perte_phys_total:.4f}  |  val: {perte_val:.4f}")
...
ax.plot(historique_phys,  label="Physique (PINN)")
ax.axhline(plancher_phys, color="gray", ls=":", lw=1, label="Plancher physique")
...
y_true_n = df_test[OUTPUTS_N].values  # Cibles normalisées pour la MSE
y_true   = df_test[OUTPUTS].values    # Valeurs physiques pour les axes du graphe
...
t_pulse = 2 * np.pi / omega          # une seule période d'oscillation imposée au bord
def u_right(t):
    return A * np.sin(omega * t) if t < t_pulse else 0.0
...
for n in range(history_needed, Nt - N_FWD * ndt + 1, N_FWD*ndt):
    ...
    # On écrit les N_FWD horizons prédits, avec conditions aux limites
    for h in range(1, N_FWD + 1):
        s = n + h*ndt
...
col_uxx = None
for cell in ws[1]:
    if cell.value == "Ut_Uxx_PF_Lap_PINN":
        col_uxx = cell.column
        break
if col_uxx is None:
    raise ValueError("Colonne 'Ut_Uxx_PF_Lap_PINN' introuvable dans la première ligne.")
col_time = col_uxx - 1 
...
def fd_once():
    ...
    for k in range(Nt):
        ...
        un[:i_left+1] = 0.0; un[i_right:] = u_right((k+1)*dt)
...
def rollout_once():
    ...
    for n in range(history_needed, Nt - N_FWD*ndt + 1, N_FWD*ndt):
        ...
        for h in range(1, N_FWD+1):
            s = n + h*ndt
            U_[s, nodes] = U_[n, nodes] + dd[:, h-1]; U_[s, :i_left+1] = 0.0; U_[s, i_right:] = u_right(s*dt)
    return U_
