# Variante Conv1d de commun.Reseau : au lieu d'aplatir tout le stencil en
# scalaires independants, on traite (lag x field) comme des canaux et les
# points du stencil comme un axe spatial, pour donner au reseau une notion
# explicite de voisinage. forward(X) -> Y garde la meme signature que
# commun.Reseau : X de forme (n_nodes*b, n_features), Y de forme
# (n_nodes*b, n_fwd) -- aucun autre fichier de commun.py n'a besoin de
# changer (meme convention que Model_in_tests/solver_gnn/.../model.py).
import torch
...
class ReseauConv(nn.Module):
    def __init__(self, n_lags: int, n_points: int, n_fields: int, n_outputs: int,
        ...
        for taille in conv_channels:
            ...
            couches_conv.append(nn.GELU())
            taille_entree = taille
        self.conv = nn.Sequential(*couches_conv)

        self.head = nn.Sequential(
            nn.Linear(conv_channels[-1] * n_points, hidden_size), nn.GELU(),
