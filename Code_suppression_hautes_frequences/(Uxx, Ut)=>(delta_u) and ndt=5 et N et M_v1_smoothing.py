# ============================================================
# VARIANTE : v1_smoothing — Laplacian smoothing renforcé
#
# Méthode testée (ISOLÉE, tout le reste = baseline) :
#   Le lissage Laplacien existait déjà dans le script original, mais
#   seulement dans la boucle de rollout principale — PAS dans reconstruct(),
#   la fonction qui construit la "fausse" prédiction auto-régressive utilisée
#   par le pushforward pendant l'ENTRAÎNEMENT. Résultat : le réseau était
#   entraîné (via pushforward) à corriger des entrées non lissées, alors
#   qu'au vrai rollout il ne voit que des champs déjà lissés — un mismatch
#   train/rollout. Ici :
#     1) le même bloc de lissage est ajouté dans reconstruct() (voir plus
#        bas), pour que pushforward et rollout appliquent la même opération
#        à chaque bloc,
#     2) SMOOTH_ALPHA est augmenté de 0.20 (valeur actuelle) à 0.35 (plus
#        fort). Stabilité : le schéma u[j] += alpha*(u[j-1]-2u[j]+u[j+1])
#        équivaut à u[j] = (1-2*alpha)*u[j] + alpha*(u[j-1]+u[j+1]) — une
#        moyenne pondérée à coefficients positifs, donc stable, tant que
#        alpha <= 0.5. 0.35 reste dans cette zone stable.
...
NOISE_STD    = 0.10
SMOOTH_ALPHA = 0.35  # v1 : renforcé par rapport à la baseline (0.20). Stable tant que alpha <= 0.5.
...
def reconstruct(u_curr, n_curr, pred_norm, A, omega):
    ...
    for h in range(1, N_FWD + 1):
        ...
        # v1 : même lissage Laplacien qu'au rollout réel, appliqué ici aussi
        # pour que le pushforward (entraînement) et le rollout (inférence)
        # traitent les blocs de façon identique.
        if SMOOTH_ALPHA > 0:
            j0, j1 = i_left + 1, i_right
            lap = u[j0-1:j1-1] - 2*u[j0:j1] + u[j0+1:j1+1]
            u[j0:j1] += SMOOTH_ALPHA * lap
