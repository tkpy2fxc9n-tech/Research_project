...
# Runs live under the phase folder they belong to, named after what that
# phase does rather than after its number: <phase>/runs/<run_id>/, e.g.
# stencil_and_features/runs/p2_reference/. Run ids keep their pN_ prefix (it
# ties a run to the report's phase ordering), but the folder name no longer
# encodes it, so the path is found by searching for the run_id rather than
# derived from a regex on it. Results are cached: every table script resolves
# the same handful of run_ids repeatedly, and the search walks the whole tree.
_RUN_DIR_CACHE: dict[Path, dict[str, Path]] = {}

# Directories that never contain a live run: superseded copies kept for
# history (which duplicate a run_id that also exists at its current path --
# see archives/superseded_runs/) and the source tree itself.
_SKIP_DIRS = {".git", "archives", "src", "common", "scripts", "checks", "dataset", "__pycache__"}


def _discover_runs(repo_root: Path) -> dict[str, Path]:
    if repo_root not in _RUN_DIR_CACHE:
        found: dict[str, Path] = {}
        for cfg in repo_root.glob("*/**/runs/*/config.yaml"):
            if any(part in _SKIP_DIRS for part in cfg.relative_to(repo_root).parts):
                continue
            found[cfg.parent.name] = cfg.parent
        _RUN_DIR_CACHE[repo_root] = found
    return _RUN_DIR_CACHE[repo_root]


def run_dir(repo_root: Path, run_id: str) -> Path:
    found = _discover_runs(repo_root).get(run_id)
    if found is None:
        # Keep returning a path (rather than raising) so callers that only
        # probe for existence behave as they did when this was a pure
        # string-derived path: load_run_metrics warns and skips a missing run.
        return repo_root / "__missing__" / run_id
    return found


def load_run_metrics(repo_root: Path, run_id: str) -> dict | None:
    # Two results-file formats coexist: results.yaml (current) and
    # metrics.json (p1_pushforward/p1_bptt only). Returns None (with a
    # WARNING, never an exception) if the run folder, its results file, or
    # its curves don't exist yet -- a run still training/queued shouldn't
    # block the other rows.
    run_path = run_dir(repo_root, run_id)
...
def load_multi_summary(repo_root: Path, run_id: str) -> dict | None:
    ...
    candidate = run_dir(repo_root, run_id)
