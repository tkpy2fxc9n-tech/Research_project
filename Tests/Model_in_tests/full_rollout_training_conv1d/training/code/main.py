...
# reusing training.py's train_model/make_dataloaders (same training loop the
# other, non-rollout full_rollout_training* projects use) instead of
...
#
# Boundary conditions are restricted to the 7 signal families from
# scenarios.py's design table -- fourier, sinusoid, chirp, gaussian, shock,
# filtered_random, free_evolution -- instead of this project's original
# single fixed left=clamped / right=Gaussian-pulse forcing. Dataset
# generation (waves.py/scenarios.py/physics.py/dataset.py) and the
# post-training evaluation/plotting (evaluation.py, the per-family showcase
# below) are unchanged by this training-scheme swap -- only HOW the weights
# get updated changed, not what data is used or how the trained model gets
# evaluated.
import argparse
import resource
import sys
...
from waves import BC_WAVEFORMS, bc_describe
from physics import run_fd_simulation_general, run_fd_simulation_free, autoregressive_rollout, biais_repos
...
# Sampling range (AMP_MIN/MAX, OMEGA_MIN/MAX) left at Config's defaults: the
# 7 signal families provide the input diversity, not a widened
# amplitude/frequency range.

N_BC_SAMPLES = 200  # number of randomly-sampled scenarios across the 7 signal families
...
def parse_args():
    ...
    p.add_argument("--epochs", type=int, default=None,
                    help="Number of epochs (default: 2 in --smoke-test, otherwise Config.N_EPOCHS). Plain "
                         "teacher-forcing is much cheaper per epoch than the old full-rollout scheme.")
...
# <family> -- same convention as test/test.py) and render a
...
def run_family_showcase(modele, cfg, norm_stats, INPUTS, OUTPUTS, output_dir: Path) -> dict:
    ...
    family_summary = {}
    # Only showcase families the model was actually trained on -- a
    # zero-weight entry in FAMILY_SHARES (e.g. to restrict training to a
    # subset of families) means the network never saw that signal shape, so
    # rendering it here would just be a meaningless rollout on an
    # out-of-distribution input.
    for family, weight in scenarios.FAMILY_SHARES.items():
        if weight <= 0:
            continue
...
def main():
    ...
    print(f"=== full_rollout_training_conv1d [{mode}] — input fields: {INPUT_FIELDS} — "
          ...
          f"{cfg.N_EPOCHS} epochs (early-stop patience={patience}), plain one-step training, "
    ...
    df, FIELDS, INPUTS, OUTPUTS = dataset.generate_dataset_multisignal(INPUT_FIELDS, cfg, scenario_list)
    ...
    df, _, _, _, rollout_idx = dataset.split_by_simulation(bc_pairs, df, cfg)
    ...
    mu_out = norm_stats.loc[OUTPUTS, "mean"].values.astype(np.float32)
    sd_out = norm_stats.loc[OUTPUTS, "std"].values.astype(np.float32)
    train_result = training.train_model(modele, train_loader, X_val, y_val, cfg,
                                         model_path=PROJECT_DIR / "model.pth", patience=patience)
    training.plot_training_curve(train_result, OUTPUT_DIR)

    df_test = df[df["split"] == "test"].reset_index(drop=True)
    tf_metrics = training.evaluate_teacher_forcing(modele, df_test, INPUTS, OUTPUTS, norm_stats, OUTPUT_DIR)

    rollout = evaluation.run_rollout(modele, FIELDS, bc_pairs, rollout_idx, INPUT_FIELDS, norm_stats,
                                      INPUTS, OUTPUTS, cfg)
    ...
    bench = evaluation.benchmark_inference(modele, FIELDS, INPUT_FIELDS, norm_stats, INPUTS, OUTPUTS, rollout, cfg)
    ...
    extra_info = {
        ...
        "CLI args": f"epochs={cfg.N_EPOCHS}, n_samples={n_samples}, batch_size={cfg.BATCH_SIZE}, "
