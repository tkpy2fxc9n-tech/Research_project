# ============================================================
# VARIANTE : v5_pushforward — pushforward renforcé
#
# Vérification (demandée dans la consigne) : le pushforward est DÉJÀ actif
# et câblé dans le script original — voir pushforward_loss(), reconstruct()
# et PF_SAMPLES, utilisés dans la boucle d'entraînement via
# `total_loss = data_loss + lam_pf * pf_loss` avec lam_pf qui monte
# progressivement de 0 à LAMBDA_PF sur PF_WARMUP=2 époques. Rien à câbler.
#
# Méthode testée (ISOLÉE, tout le reste = baseline) :
#   On augmente uniquement le poids du pushforward : LAMBDA_PF passe de 1.0
#   (baseline) à 3.0. Le warmup (PF_WARMUP=2) est laissé inchangé pour
#   n'isoler que l'effet du poids final. NOISE_STD (0.10) et SMOOTH_ALPHA
#   (0.20) restent aux valeurs de la baseline, et reconstruct() n'est PAS
#   modifié ici (contrairement à v1/v4) pour ne changer qu'un seul facteur
#   à la fois.
#
...
LAMBDA_PF   = 3.0   # v5 : renforcé par rapport à la baseline (1.0)
