# Method U_Ut_Uxx_pf40: U + Ut + Uxx as input (pipeline shared in commun.py), but
# with pushforward unrolled over 40 hops instead of 3 -- isolated experiment to
# test whether a much longer training horizon better stabilizes the full
# rollout (54 blocks). Does not modify methode_U_Ut_Uxx/.
import sys
from pathlib import Path

SCRIPT_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(SCRIPT_DIR.parent))
import Archives.Archives.comparaisons.comparaison_des_inputs.commun as C

INPUT_FIELDS = ["U", "Ut", "Uxx"]
METHOD_NAME = "U_Ut_Uxx_pf40"
...
def main():
    cfg = C.Config(PF_HOPS=40)   # only change vs methode_U_Ut_Uxx: everything else identical
    C.set_seeds(cfg)
