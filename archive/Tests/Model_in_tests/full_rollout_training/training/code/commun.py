# Simulation/training/rollout/export logic for this project's one method
# (see config.py for INPUT_FIELDS/N_SCENARIOS and every tunable parameter --
# Config itself is defined there, not here; see scenarios.py for which BC
# families are actually sampled).
#
# Vendored from Tests/Archives/comparaisons/comparaison_des_inputs/commun.py
# and trimmed to only what full_rollout_training actually calls. Boundary
# conditions are generalized (dirichlet, either end, arbitrary waveform
# family -- see run_fd_simulation_general/apply_boundary_conditions below),
# restricted by scenarios.py to the "gaussian"/"rest" families -- the fixed
# left=0/right=Gaussian-pulse physics this project started with is gone, not
# just unused, since it's a strict special case of the general one. Also
# dropped because nothing here reaches them: the pushforward training loop
# (train.py has its own full-rollout TBPTT loop instead), the row-level
# split_and_normalize/make_dataloaders path (data_split.py splits by
# simulation instead), and the xlsx comparison-table exporters.
...
# Each simulation is run independently: parallelizable over a process pool
# (uses the cpus allocated by Slurm).
...
# ============================================================================
# Generalized boundary conditions: each end independently imposes a
# displacement (dirichlet) following one of the families in BC_WAVEFORMS,
# restricted by scenarios.py to "gaussian" and "rest". A boundary condition
# is (bc_type, waveform_family, params) -- bc_type is always "dirichlet"
# here (kept as an explicit field, not hardcoded away, so BC_WAVEFORMS/
# apply_boundary stay reusable if a Neumann family is ever added).
# ============================================================================
...
def _sample_amplitude(rng, cfg: Config) -> float:
    ...
    # sampled "A" carries this precision for real, not just in printouts.
...
def apply_boundary(u: np.ndarray, side: str, value: float, cfg: Config) -> np.ndarray:
    # Dirichlet: fill the whole ghost band + boundary node with `value` --
    # same as the original fixed-BC code (u_new[:i_left+1]=.../
    # u_new[i_right:]=...), just with a per-scenario value instead of a
    # hardcoded 0 (left) / u_right_val (right).
    i_left, i_right = cfg.i_left, cfg.i_right
    if side == "left":
        u[:i_left + 1] = value
    else:
...
def apply_boundary_conditions(u: np.ndarray, t: float, bc_left: BCSpec, bc_right: BCSpec, cfg: Config) -> np.ndarray:
    apply_boundary(u, "left", bc_value(bc_left, t), cfg)
    apply_boundary(u, "right", bc_value(bc_right, t), cfg)
    return u


def run_fd_simulation_general(bc_left: BCSpec, bc_right: BCSpec, cfg: Config) -> np.ndarray:
    # Same leapfrog scheme as the original run_fd_simulation, ghost-fill via
    # the shared apply_boundary_conditions helper instead of two hardcoded lines.
...
def generate_dataset_general(input_fields: list[str], cfg: Config, bc_pairs: list[tuple[BCSpec, BCSpec]],
    ...
    # Same parallelization pattern as the old generate_dataset, threaded
    # through a list of (left_bc, right_bc) pairs (random sample, not a
    # dense grid). FIELDS/df are keyed by simulation index (bc dicts aren't
    # hashable, unlike a plain (A, omega) tuple).
...
def reconstruct_general(u_curr, n_curr, pred_norm, bc_left: BCSpec, bc_right: BCSpec, mu_out, sd_out,
    ...
    # Numpy reference used by check_equivalence.py to validate
    # rollout_torch.reconstruct_torch (baseline fixed across h, not chained
    # -- unlike the FD simulation, we never chain h=1 into h=2 here).
...
def _autoregressive_rollout_general(modele, U_reel, input_fields, mu_in, sd_in, mu_out, sd_out,
    ...
    # Reused by run_rollout_general and the benchmark. The first M_BACK*ndt
    # steps come from U_reel (history needed before the 1st prediction),
    ...
    for n in range(history_needed, cfg.Nt - cfg.N_FWD*cfg.ndt + 1, cfg.N_FWD*cfg.ndt):
        m_list = [n - lag*cfg.ndt for lag in range(cfg.M_BACK)]
        X = np.clip((build_window(m_list, lambda m: U[m], input_fields, cfg) - mu_in) / sd_in,
                    -cfg.INPUT_CLIP, cfg.INPUT_CLIP)
        with torch.no_grad():
...
def bc_describe(bc: BCSpec) -> str:
    ...
    # are untouched, so the simulation/rollout math is unaffected).
...
def _input_fields_from_columns(INPUTS) -> list[str]:
    ...
    # separate argument (keeps export_resume_general's call sites unchanged).
...
def _write_resume_body(f, cfg: Config, method_name: str, df: pd.DataFrame, INPUTS, OUTPUTS,
    ...
    f.write(f"BC sampling     : each end independently gaussian (A in [{cfg.AMP_MIN}, {cfg.AMP_MAX}], "
            f"omega in [{cfg.OMEGA_MIN}, {cfg.OMEGA_MAX}]) or rest, dirichlet\n")
