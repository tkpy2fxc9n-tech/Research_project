"""
Surrogate — équation des ondes 1D.
...
def u_right_val(A, omega, t):
    ...
    if t < t_pulse:
        signal = A * np.sin(omega * t) + 0.05 * np.sin(10 * omega * t)
...
for A, omega in product(AMPLITUDES, PULSATIONS):
    ...
    def u_right(t):
        if t < t_pulse :
            return A * np.sin(omega * t) + 0.05*np.sin(omega*2*t)
        else :
            return 0.0    
...
def build_window(m_list, field_at):
    """Construit l'entrée normalisée (n_nodes, n_inputs), dans le MÊME ordre de
    colonnes que le dataset (M_BACK lags × voisinage ±SS).
    m_list = instants des lags ; field_at(m) = champ (vrai ou prédit) au pas m."""
...
def reconstruct(u_curr, n_curr, pred_norm, A, omega):
    """Depuis les deltas prédits (normalisés) au temps n_curr, reconstruit les
    champs prédits aux N_FWD horizons, avec les conditions aux limites."""
    deltas = pred_norm * sd_out + mu_out             # (n_nodes, N_FWD), physique
    champs = {}
    for h in range(1, N_FWD + 1):
        s = n_curr + h * ndt
        u = np.zeros(Ntot)
...
def pushforward_loss(n_groups):
    """Déroule un bloc N/M :
       - Bloc 1 (SANS gradient) : le réseau prédit à partir d'entrées vraies,
         on reconstruit un champ prédit -> état "abîmé" par ses propres erreurs.
       - Bloc 2 (AVEC gradient) : on repart de cet état abîmé et on demande au
         réseau de revenir vers la vérité terrain.
    Le détachement entre les deux blocs est l'astuce du pushforward."""
    idxs = np.random.choice(len(PF_SAMPLES), n_groups, replace=False)
    groups = [PF_SAMPLES[i] for i in idxs]

    # --- Bloc 1 : on pousse le modèle sur ses propres sorties (détaché) ---
    ...
    # --- Bloc 2 : entrée reconstruite depuis l'état prédit, cible = vérité ---
    nN = len(nodes)
    X2_list, tgt_list = [], []
