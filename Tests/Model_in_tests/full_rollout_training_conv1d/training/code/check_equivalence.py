...
# full_rollout_training_multisignal) faithfully reproduces the numpy physics
# already validated in physics.py/waves.py, over a few hops and a Conv1d model with
# fixed random weights, for every signal family this project trains on
# (including the fourier/chirp/shock/filtered_random ones, and the
# free-evolution random-initial-state case). Does NOT test the gradient
# (only the values) -- the goal is to catch a transcription error before
...
def run_case(cfg, modele, input_fields, mu_in, sd_in, mu_out, sd_out, biais_repos,
    ...
    # --- Reference: numpy rollout (physics.py), truncated to N_HOPS_TEST hops ---
...
def main():
    ...
    # at rest (no push) -- exercises physics.run_fd_simulation_free, the one
    # genuinely new physics function this project adds.
