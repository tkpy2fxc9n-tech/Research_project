...
# generalized-boundary-condition physics validated in commun.py
...
def apply_boundary_torch(u_row: torch.Tensor, side: str, bc_type: str, value: float, cfg: "C.Config") -> torch.Tensor:
    # u_row: 1D tensor (Ntot,). Returns a NEW tensor (no in-place ops) --
    # exact torch counterpart of C.apply_boundary, same index conventions
...
def apply_boundary_conditions_torch(u: torch.Tensor, t: float, bc_left_list, bc_right_list,
    ...
    for g in range(u.shape[0]):
        row = u[g]
        left_val = C.bc_value(bc_left_list[g], t)
        right_val = C.bc_value(bc_right_list[g], t)
