# ============================================================
# VARIANTE : v3_spectral_penalty — pénalité de norme spectrale (1ère couche)
#
# Méthode testée (ISOLÉE, ajoutée EN PLUS de la baseline — NOISE_STD=0.10,
# SMOOTH_ALPHA=0.20, LAMBDA_PF=1.0 restent inchangés) :
#   On pénalise directement le GAIN de la première couche du MLP sur le mode
#   de Nyquist spatial (le damier), plutôt que de traiter le symptôme après
#   coup (lissage) ou de bruiter les entrées (v2). C'est l'option "norme
#   spectrale sur les poids de la 1ère couche" suggérée dans la consigne.
#
#   Calcul : soit W1 la matrice de poids de la 1ère couche (shape
#   [hidden0, n_inputs]) et c la direction "damier" unitaire dans l'espace
#   des features normalisées (même construction que v2 : pattern (-1)^k sur
#   les voisins -SS..+SS, dupliqué pour u_dot/u_xx, répété sur M_BACK lags,
#   puis normalisé ||c||=1). Le "gain damier" de chaque neurone caché de la
#   1ère couche est g = W1 @ c (produit matrice-vecteur, shape [hidden0]) :
#   c'est la réponse de chaque neurone à une entrée purement "damier"
#   d'amplitude unité. On ajoute à la loss :
#       LAMBDA_SPEC * mean(g^2)
#   ce qui pousse le réseau à réduire sa sensibilité au mode damier
#   spécifiquement, sans contraindre sa réponse aux basses fréquences
#   (l'onde physique) qui vivent dans une direction quasi orthogonale à c
#   dans l'espace des features. LAMBDA_SPEC=1e-3 est choisi petit devant la
#   loss de données (MSE normalisée, échelle ~1e-2 à 1e-3 en cours
#   d'entraînement) pour agir comme régularisation douce, pas comme
#   contrainte dominante ; à ajuster si l'effet est trop faible/fort.
#
...
VARIANT_NAME = "v3_spectral_penalty"
...
# v3 : poids de la pénalité de norme spectrale (gain de la 1ère couche sur le
# mode damier / Nyquist spatial) — voir explication en tête de fichier.
LAMBDA_SPEC = 1e-3
...
# --- v3 : direction "damier" unitaire dans l'espace des features normalisées
# (même construction que v2, voir explication en tête de fichier), utilisée
# pour pénaliser le gain de la 1ère couche sur ce mode précis.
_offset_sign  = np.array([(-1.0) ** i for i in range(2*SS + 1)], dtype=np.float32)
_checker_bloc = np.repeat(_offset_sign, 2)
checker_dir   = np.tile(_checker_bloc, M_BACK).astype(np.float32)
checker_dir   = checker_dir / np.linalg.norm(checker_dir)
assert len(checker_dir) == len(INPUTS), "checker_dir ne correspond pas à l'ordre des colonnes de INPUTS"
checker_dir_t = torch.tensor(checker_dir)
...
def spectral_hf_penalty():
    """v3 : gain de la 1ère couche sur le mode damier (Nyquist spatial)."""
    W1 = modele.reseau[0].weight              # (hidden0, n_inputs)
    gain_checker = W1 @ checker_dir_t          # réponse de chaque neurone au mode damier pur
    return (gain_checker ** 2).mean()
...
for epoch in range(1, N_EPOCHS + 1):
    ...
    for X_batch, y_batch in train_loader:
        ...
        # --- v3 : pénalité de norme spectrale (gain de la 1ère couche sur le
        #     mode damier) ---
        spec_loss = spectral_hf_penalty()

        total_loss = data_loss + lam_pf * pf_loss + LAMBDA_SPEC * spec_loss
        total_loss.backward()
        optimiseur.step()

        perte_train      += data_loss.item()
        perte_pf_total   += pf_loss.item()
        perte_spec_total += spec_loss.item()

    perte_train      /= len(train_loader)
    perte_pf_total   /= len(train_loader)
    perte_spec_total /= len(train_loader)
    ...
    historique_spec .append(perte_spec_total)

    if epoch % 1 == 0:
        print(f"Epoch {epoch:4d}/{N_EPOCHS}  —  "
              f"data: {perte_train:.4f}  |  pushf: {perte_pf_total:.4f}  |  "
              f"spec: {perte_spec_total:.4f}  |  val: {perte_val:.4f}")
...
ax.plot(historique_spec,  label="Pénalité spectrale (v3)")
ax.set_xlabel("Epoch"); ax.set_ylabel("Loss")
ax.set_title("Courbe d'apprentissage (pushforward + pénalité spectrale)")
