"""
Quick topology test: N0--N1--N2 in series (1 degree-2 junction, N1), then at
N2 the network branches into two more rods N2--N3 and N2--N4 (N2 becomes a
degree-3 junction, a real branch). N3/N4 are clamped ends. N0 is the driven
(gaussian pulse) end.

Adapted from three_rod_series_test.py (topology/NN-surrogate machinery,
which already generalizes over any junction degree) + the degree>=3
"continuity + zero net force at a massless junction" FD stencil from
graph_rods_3d_prototype.py (three_rod_series_test.py's own FD code only
ever handled degree-2 junctions).

Run it with:
    python3 branch_after_series_test.py
Writes branch_after_series.gif, branch_after_series_amplitude_vs_time.png,
and branch_after_series_amplitude_trace.npz next to this script.
"""
...
# --- Topology: N0(driven, gaussian) -- N1 -- N2(branch) -- N3(clamped)
#                                                        \-- N4(clamped)
# Angled purely for the 3D render; physics only cares about arc-length
# along each rod.
PULSE_A = cfg.AMP_MAX
PULSE_SIGMA = 0.3
pos = {"N0": np.array([0.0, 0.0])}
pos["N1"] = pos["N0"] + cfg.L * np.array([np.cos(np.radians(0.0)), np.sin(np.radians(0.0))])
pos["N2"] = pos["N1"] + cfg.L * np.array([np.cos(np.radians(35.0)), np.sin(np.radians(35.0))])
pos["N3"] = pos["N2"] + cfg.L * np.array([np.cos(np.radians(-25.0)), np.sin(np.radians(-25.0))])
pos["N4"] = pos["N2"] + cfg.L * np.array([np.cos(np.radians(80.0)), np.sin(np.radians(80.0))])
NODE_NAMES = ["N0", "N1", "N2", "N3", "N4"]
# Edge direction matters here (unlike plain physics): the "simple" dataset
# this model was trained on used REST_SHARES_SIMPLE={"left_rest": 1.0} --
# EVERY training rod had rest on its own left/start and the driven pulse on
# its own right/end, never the reverse. So the pulse-adjacent rod must have
# the pulse on its .end, and the clamped rods must have the clamp on their
# .start, or the model sees an input arrangement it never saw in training.
# The middle rod (both ends are junctions, no directly-imposed BC) has no
# such constraint.
# Pulse now moved to N3 (one of the branch tips) instead of N0 -- N0 and N4
# are both clamped instead. Tests whether exciting FROM the branch side
# (through the degree-3 junction first, then the degree-2 one) changes the
# late-time blow-up found when exciting from the single-rod side.
edges = [("N0", "N1"), ("N1", "N2"), ("N2", "N3"), ("N4", "N2")]

nodes = {
    "N0": {"pos": pos["N0"], "driven": True, "bc": ("dirichlet", "rest", {})},   # clamped
    "N1": {"pos": pos["N1"], "driven": False},
    "N2": {"pos": pos["N2"], "driven": False},   # degree-3 junction: real branch
    "N3": {"pos": pos["N3"], "driven": True, "bc": ("dirichlet", "gaussian", {"A": PULSE_A, "sigma": PULSE_SIGMA})},
    "N4": {"pos": pos["N4"], "driven": True, "bc": ("dirichlet", "rest", {})},   # clamped
}
...
# ============================== FD reference ==============================
# Degree-2 junction (N1): exact-continuity 3-point stencil, same as
# three_rod_series_test.py. Degree-3 junction (N2): continuity + zero net
# force at a massless junction, averaged -- same formula as
# graph_rods_3d_prototype.py's real-branch case.
...
# ============================== NN surrogate ================================
# Ghost bands borrowed from every OTHER rod touching a node, averaged --
# already generic over junction degree (no change needed vs
# three_rod_series_test.py for the degree-3 case).
...
print("\nPeak |u| per rod -- FD vs NN (distance = number of junctions from the driven end N3):")
dist_from_source = {("N2", "N3"): 0, ("N1", "N2"): 1, ("N4", "N2"): 1, ("N0", "N1"): 2}
...
np.savez(SCRIPT_DIR / "branch_after_series_amplitude_trace.npz",
         fd_t=fd_t, nn_t=nn_t,
...
fig.suptitle("N0-N1-N2 in series (degree-2 junction N1), N2 branches into N3/N4 (degree-3): FD vs NN")
fig.tight_layout()
fig.savefig(SCRIPT_DIR / "branch_after_series_amplitude_vs_time.png", dpi=120)
print(f"Saved {SCRIPT_DIR / 'branch_after_series_amplitude_vs_time.png'}")
...
anim.save(SCRIPT_DIR / "branch_after_series.gif", writer="pillow", fps=25)
print(f"Saved {SCRIPT_DIR / 'branch_after_series.gif'}")
