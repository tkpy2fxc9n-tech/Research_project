#!/usr/bin/env python3
"""
p7_complex_zeroshot: evaluate an already-trained model (trained on the
...
see its own env.json/logs), not interactively on a login node:
    sbatch --job-name=p7_complex_zeroshot --mem=<match source run> \\
...
from beamsurrogate.config import Config, config_to_dict, set_seeds  # noqa: E402
...
from beamsurrogate.evaluate.metrics import build_metrics, write_metrics_json  # noqa: E402
from beamsurrogate.evaluate import plots  # noqa: E402
from beamsurrogate.cli import write_env_json  # noqa: E402

DATA_DIR = REPO_ROOT / "data"
RUNS_DIR = REPO_ROOT / "runs"

RUN_ID = "p7_complex_zeroshot"
# TBD -- set to whichever LAMBDA_PHYSICS weight wins the p3_pinn_* (H6) sweep,
...
def main():
    if SOURCE_RUN_ID is None:
        print("ERROR: SOURCE_RUN_ID is not set -- edit this script once the p3_pinn_* (H6) sweep "
              "has a winner, then rerun.", file=sys.stderr)
        sys.exit(1)
    source_model_path = RUNS_DIR / SOURCE_RUN_ID / "model.pth"
    if not source_model_path.exists():
        print(f"ERROR: {source_model_path} not found -- {SOURCE_RUN_ID} may not have finished.",
              file=sys.stderr)
        sys.exit(1)

    cfg_source = _load_resolved_config(SOURCE_RUN_ID)
    ...
    # (architecture/windowing) except dataset + run_id/phase/hypothesis, so
    # INPUTS/OUTPUTS column layout matches the model exactly.
    cfg_target = dataclasses.replace(cfg_source, dataset=TARGET_DATASET, run_id=RUN_ID,
                                      phase=7, hypothesis=None)
    ...
    with open(run_dir / "config.resolved.yaml", "w") as f:
        yaml.safe_dump(config_to_dict(cfg_target), f, sort_keys=False)
    write_env_json(run_dir / "env.json", complex_path)
    ...
    summary = (
        f"{RUN_ID} (phase 7) -- zero-shot: {SOURCE_RUN_ID}'s model (trained on simple), "
