# Variante diagnostic de Code_comparaison_des_inputs/commun.py : les deux
# bords sont encastrés (u=0 fixe, plus de forçage au bord droit) et la
# simulation démarre d'une bosse gaussienne spatiale au centre du domaine au
# lieu d'un domaine au repos excité par le bord. Tout le reste (schéma FD,
# CFL, réseau, pushforward, hyperparamètres, grille de scénarios) est
# identique à l'original pour que la comparaison au cas forcé soit valide.
...
class Config:
    ...
    AMP_MIN: float = 0.005
    AMP_MAX: float = 0.1
    # OMEGA_MIN/OMEGA_MAX/PULSATIONS/ROLLOUT_OMEGA_IDX : plus de forçage
    # temporel dans ce régime, cet axe est réutilisé comme largeur spatiale
    # de la bosse initiale (via largeur_from_omega), avec les mêmes valeurs
    # numériques que l'original pour garder une grille N_GRID x N_GRID
    # comparable.
    OMEGA_MIN: float = 3
    OMEGA_MAX: float = 10
...
def largeur_from_omega(omega: float) -> float:
    # Même mapping que l'ancien sigma temporel (u_right_val dans
    # Code_comparaison_des_inputs), réutilisé ici comme largeur spatiale de
    # la bosse initiale.
    return np.interp(omega, [1.0, 10.0], [0.15, 0.07])


def initial_bump(A: float, omega: float, cfg: Config) -> np.ndarray:
    i_left, i_right, Ntot = cfg.i_left, cfg.i_right, cfg.Ntot
    largeur = largeur_from_omega(omega)
    x = (np.arange(Ntot) - i_left) * cfg.dx
    x0 = cfg.L / 2
    u0 = A * np.exp(-((x - x0) / largeur) ** 2)
    u0[:i_left+1] = 0.0
    u0[i_right:] = 0.0
    return u0
...
def run_fd_simulation(A: float, omega: float, cfg: Config) -> np.ndarray:
    # Grille étendue avec SS points fantômes : bords gauche ET droit
    # encastrés (=0), plus aucun forçage externe. Condition initiale : bosse
    # gaussienne spatiale au centre du domaine, vitesse initiale nulle.
    i_left, i_right, Ntot = cfg.i_left, cfg.i_right, cfg.Ntot
    u_storage = np.zeros((cfg.Nt + 1, Ntot))
    u = initial_bump(A, omega, cfg)
    u_1 = u.copy()
    u_storage[0] = u.copy()
