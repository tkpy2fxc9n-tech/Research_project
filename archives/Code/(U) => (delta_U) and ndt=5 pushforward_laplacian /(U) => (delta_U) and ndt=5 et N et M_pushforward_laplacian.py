...
for idx, (A, omega) in enumerate(product(AMPLITUDES, PULSATIONS), start=1):
    ...
    for n in range(ndt, Nt - ndt + 1):
        ...
        u_t  = u_storage[n]        
        u_tm = u_storage[n - ndt]    

        for k in range(-SS, SS + 1):              
            row[f"u_t({jlabel(k)})"] = u_t[nodes + k]
        row["u_tm(j)"] = u_tm[nodes]            

        row["delta_u"] = u_storage[n + ndt, nodes] - u_t[nodes]  
...
def u_right_val(A, omega, t):
    t_pulse = 2 * np.pi / omega
    return (A*np.sin(omega*t) + 0.05*np.sin(omega*2*t)) if t < t_pulse else 0.0
...
# Roll out cell  (avance par pas de ndt)
import time
...
# Erreur en fonction du temps, sur les indices reellement remplis
