# FD leapfrog solver (generalized arbitrary Dirichlet/Neumann boundary
# conditions via waves.py) and the autoregressive rollout used at
# evaluation/inference time. Ported from Tests/Model_in_tests/
# full_rollout_training_conv1d/training/code/physics.py.
...
from ..data.windows import build_window
...
def run_fd_simulation_free(bc_left: BCSpec, bc_right: BCSpec, u0_profile: np.ndarray, cfg) -> np.ndarray:
    # "Free evolution": both ends stay at rest (no push); instead the beam
    # *starts* bent into a random smooth shape and is simply let go. Same
    # leapfrog scheme as run_fd_simulation_general, except u/u_1 start from
    # u0_profile (zero initial velocity) instead of zero displacement.
    i_left, i_right, Ntot = cfg.i_left, cfg.i_right, cfg.Ntot
    u_storage = np.zeros((cfg.Nt + 1, Ntot))
...
def reconstruct_general(u_curr, n_curr, pred_norm, bc_left: BCSpec, bc_right: BCSpec, mu_out, sd_out,
    ...
    # Numpy reference reconstruction -- used directly by the equivalence
    # check to validate the differentiable torch port (training/losses.py's
    # torch physics module).
