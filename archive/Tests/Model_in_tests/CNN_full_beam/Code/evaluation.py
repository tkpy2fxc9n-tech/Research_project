...
from physics import biais_repos as compute_biais_repos, autoregressive_rollout, run_fd_simulation_general
...
def run_rollout(modele, FIELDS: dict, bc_pairs: list[tuple[BCSpec, BCSpec]], rollout_idx: int,
                norm_stats, INPUT_CHANNELS, OUTPUT_CHANNELS, cfg) -> RolloutResult:
    ...
    U = autoregressive_rollout(modele, U_reel, mu_in, sd_in, mu_out, sd_out,
...
def benchmark_inference(modele, norm_stats, INPUT_CHANNELS, OUTPUT_CHANNELS,
    ...
    def rollout_once():
        return autoregressive_rollout(modele, U_reel, mu_in, sd_in, mu_out, sd_out,
                                       biais_repos, left_bc, right_bc, cfg)
    ...
    # One forward call = one whole-beam sample of shape (1, n_channels, Nx)
    # -- the old code's (n_nodes, n_features) batch covered the same beam,
    # just batched over nodes, so n_calls (number of rollout hops) is the
    # same count in both projects.
    from torch.utils.flop_counter import FlopCounterMode
    with FlopCounterMode(display=False) as fc:
        modele(torch.zeros((1, len(INPUT_CHANNELS), len(cfg.nodes))))
...
def export_resume(output_dir: Path, cfg, method_name: str, n_sims_by_split: dict,
    ...
    with open(output_dir / "resume.txt", "w") as f:
        ...
        f.write(f"Grid            : Nt={cfg.Nt}, Nx={cfg.Nx}, ndt={cfg.ndt} "
        ...
        f.write(f"Input channels  : {', '.join(INPUT_CHANNELS)} (U only, whole beam)\n")
        f.write(f"Spatial extent  : full beam, Nx={cfg.Nx} nodes per forward pass "
                f"(conv receptive field: 9 points -- 2 layers of kernel_size=5)\n")
        f.write(f"Sampling range  : A in [{cfg.AMP_MIN}, {cfg.AMP_MAX}], sigma in [{cfg.OMEGA_MIN}, {cfg.OMEGA_MAX}]\n")
        f.write(f"Rollout BC      : left={bc_describe(rollout.left_bc)}  right={bc_describe(rollout.right_bc)}\n")
        f.write(f"Smoothing       : SMOOTH_ALPHA={cfg.SMOOTH_ALPHA}\n")
        f.write(f"Seeds           : SEED={cfg.SEED}, SPLIT_SEED={cfg.SPLIT_SEED}\n")
        n_total = sum(n_sims_by_split.values())
        f.write(f"Simulations     : {n_total} total\n")
        for s in ["train", "val", "test"]:
            n = n_sims_by_split[s]
            f.write(f"  split {s:5s}   : {n:>4d} simulations ({100*n/n_total:.1f} %)\n")
        ...
        f.write(f"Minimum val     : {train_result.meilleure_val:.6e} (L2 rel rollout error)\n")
        for col, m in tf_metrics.items():
            f.write(f"{col:15s} : MSE (norm) = {m['mse_norm']:.4e} | R2 = {m['r2']:.4f}  (one-step check)\n")
        f.write("\n")
