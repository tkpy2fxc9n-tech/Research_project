# ============================================================
# VARIANTE : v4_spectral_filter — filtre passe-bas FFT en post-traitement
#
# Méthode testée (ISOLÉE, REMPLACE le lissage Laplacien de la baseline) :
#   Après chaque bloc de rollout (comme le faisait le lissage Laplacien),
#   on applique un filtre passe-bas explicite dans le domaine de Fourier
#   spatial : FFT du champ intérieur -> mise à zéro des fréquences spatiales
#   au-delà d'une fraction CUTOFF_FRAC de la fréquence de Nyquist -> IFFT ->
#   réinjection. C'est un filtrage net (coupure brutale en fréquence),
#   différent du lissage Laplacien (atténuation progressive, plus forte près
#   de Nyquist mais non nulle ailleurs) — d'où deux variantes séparées (v1
#   vs v4) pour comparer les deux familles de post-traitement.
#   SMOOTH_ALPHA est mis à 0 (désactivé) pour isoler l'effet du filtre FFT
#   seul, sans le cumuler avec le lissage Laplacien. CUTOFF_FRAC=0.5 (on
#   garde la moitié basse du spectre spatial) est le point de départ ; à
#   ajuster si le filtrage est trop agressif (perte de l'onde physique) ou
#   trop faible (damier résiduel).
#   Comme pour v1, le même filtre est appliqué dans reconstruct() (utilisé
#   par le pushforward à l'entraînement) pour que l'entraînement voie
#   exactement ce que verra le rollout réel.
# NOISE_STD (0.10) et LAMBDA_PF (1.0) restent aux valeurs de la baseline.
#
...
VARIANT_NAME = "v4_spectral_filter"
...
NOISE_STD    = 0.10
SMOOTH_ALPHA = 0.0   # v4 : lissage Laplacien désactivé, remplacé par le filtre FFT ci-dessous

# v4 : filtre passe-bas FFT en post-traitement du rollout (remplace le
# lissage Laplacien). Fraction de la fréquence de Nyquist spatiale conservée.
CUTOFF_FRAC = 0.5

def spectral_lowpass(u_interior, cutoff_frac):
    """Filtre passe-bas FFT : ne garde que les cutoff_frac*Nyquist premières
    fréquences spatiales du champ intérieur (1D, réel)."""
    U_fft = np.fft.rfft(u_interior)
    n_keep = max(1, int(cutoff_frac * len(U_fft)))
    U_fft[n_keep:] = 0.0
    return np.fft.irfft(U_fft, n=len(u_interior))
...
def reconstruct(u_curr, n_curr, pred_norm, A, omega):
    ...
    for h in range(1, N_FWD + 1):
        ...
        # v4 : même filtre FFT passe-bas qu'au rollout réel, appliqué ici
        # aussi pour que le pushforward (entraînement) et le rollout
        # (inférence) traitent les blocs de façon identique.
        if CUTOFF_FRAC < 1.0:
            j0, j1 = i_left + 1, i_right
            u[j0:j1] = spectral_lowpass(u[j0:j1], CUTOFF_FRAC)
...
for n in range(history_needed, Nt - N_FWD * ndt + 1, N_FWD*ndt):
    ...
    for h in range(1, N_FWD + 1):
        ...
        # v4 : filtre passe-bas FFT (remplace le lissage Laplacien) : casse le
        # mode "damier" haute fréquence que u_xx amplifierait au bloc suivant,
        # sans amortir l'onde (basse fréq).
        if CUTOFF_FRAC < 1.0:
            j0, j1 = i_left + 1, i_right            # nœuds intérieurs (hors bords)
            U[s, j0:j1] = spectral_lowpass(U[s, j0:j1], CUTOFF_FRAC)
